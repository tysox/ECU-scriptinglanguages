...
#
#Calculates the hash of the secretPassword var, and prints that into a file (secret.txt) within the folderName dictated.
#The > ensures that if there is a file already there, it's overwritten.
sha256sum $secretPassword > ./$folderName/$(whoami).txt
#
